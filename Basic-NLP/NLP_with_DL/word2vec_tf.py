...
def remove_punctuation_3(s):
  '''
  we should use this Python 3 version
  '''
...
def train_model(savedir):
  ...
  emb_input = tf.nn.embedding_lookup(tfW, tf_input) # 1 x D, one center word
  emb_output = tf.nn.embedding_lookup(tfV, tf_context) # N x D, N context words
  ...
  # logit is the vector of raw (non-normalized) predictions that a classification model generates, 
  # which is ordinarily then passed to a normalization function
  pos_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                  labels=tf.ones(tf.shape(correct_output)), logits=correct_output) # tf.ones
  ...
  neg_loss = tf.nn.sigmoid_cross_entropy_with_logits(
    labels=tf.zeros(tf.shape(incorrect_output)), logits=incorrect_output) # tf.zeros
