...
if __name__ == '__main__':
  ...
  for epoch in range(epochs):
    ...
    for sentence in sentences:
      ...
      inputs = np.zeros((n - 1, V)) # first n-1(1st~(n-1)th) words as input
      targets = np.zeros((n - 1, V)) # lasr n-1(2nd~nth) words as target
      # [row, col] == [last word, current word] -- [list, list]
      ...
      # keep track of the bigram loss, no need to train W_bigram!!!
  ...
  plt.subplot(1,2,1)
  plt.title("Logistic Model Probs")
