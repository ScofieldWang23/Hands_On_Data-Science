...
def get_stories(f):
  ...
  for line in f:
    ...
    #       question<TAB>answer
    # it also tells us which line in the story is relevant to the answer -- supporting
...
# we will need to re-pad the stories later so that each story is the same length!
...
# embedding_dim = 15 # a small embedding size is fine for this simple task
...
# embedded_story = Lambda(lambda x: K.sum(x, axis=2))(embedded_story) # a list of vectors, vector: sentence embedding
...
# x = dot([embedded_story, embedded_question], 2) # (N, num sentences)
...
# # story_weights.shape = (N, num sentences, 1)
# # embedded_story.shape = (N, num sentences, embedding_dim)
...
# the "query" can be 
#   1. the question, 
#   2. the answer from the previous hop
def hop(query, story):
  # query.shape = (N, embedding_dim,)
  # story.shape = (N, num sentences, embedding_dim)
  x = Reshape((1, embedding_dim))(query) # make it (N, 1, embedding_dim)
  x = dot([story, x], 2)
  x = Reshape((story_maxsents,))(x) # flatten it for softmax
  x = Activation('softmax')(x)
  story_weights = Reshape((story_maxsents, 1))(x) # unflatten for dotting, (N, num sentences, 1)

  # makes a new embedding
  story_embedding2 = embed_and_sum(input_story_) # (N, num sentences, embedding_dim)
