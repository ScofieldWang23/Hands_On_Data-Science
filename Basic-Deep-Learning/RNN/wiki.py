...
class RNN:
    def __init__(self, D, hidden_layer_sizes, V):
        ...
        self.D = D # word vector dimension
        self.V = V # vocab size


    def fit(self, X, learning_rate=1e-5, mu=0.99, epochs=10, 
            show_fig=True, activation=T.nnet.relu, recurrent_unit=GRU, normalize=True):
        D = self.D
        V = self.V
        N = len(X)
        ...
        for Mo in self.hidden_layer_sizes:
            # activation can be a list, here for each hidden layer, we use relu
            ru = recurrent_unit(Mi, Mo, activation)  
            self.hidden_layers.append(ru)
            Mi = Mo # remember to change Mi
        ...
        for i in range(epochs):
            ...
            for j in range(N): # one sentence at a time
                if np.random.random() < 0.01 or len(X[j]) <= 1: # corner case
...
def train_wikipedia(we_file='word_embeddings.npy', w2i_file='wikipedia_word2idx.json', recurrent_unit=GRU):
    ...
    ###       just comment one out, and uncomment the other! Brown corpus is much faster in training
...
def find_analogies(w1, w2, w3, we_file='word_embeddings.npy', w2i_file='wikipedia_word2idx.json'):
    ...
    for dist, name in [(dist1, 'Euclidean'), (dist2, 'cosine')]:
        min_dist = float('inf')
        best_word = ''
        for word, idx in word2idx.items(): # iteritems
