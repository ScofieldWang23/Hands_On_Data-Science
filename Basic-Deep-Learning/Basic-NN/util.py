...
def get_transformed_data():
    ...
    if not os.path.exists(r'../large_files/train.csv'):
    ...
    df = pd.read_csv(r'../large_files/train.csv')
...
def get_normalized_data():
    ...
    if not os.path.exists('./data/digit-recognizer/train.csv'):
        print('Looking for ../data/digit-recognizer/train.csv')
        print('You have not downloaded the data and/or not placed the files in the correct location.')
        print('Please get the data from: https://www.kaggle.com/c/digit-recognizer')
        print('Place train.csv in the folder large_files adjacent to the class folder')
        exit()
...
def y2indicator(y):
    ...
    N = len(y)
    y = y.astype(np.int32)
    ind = np.zeros((N, 10)) # 10 is #classes
    for i in range(N):
        ind[i, y[i]] = 1
    return ind
